...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant ALPHA_VAR_SLOW = 7e16;
    uint256 private constant ALPHA_L = 12e16;
    uint256 private constant ALPHA_L_SLOW = 4e16;
    ...
    uint256 private constant SIGMA_STRESS_GAP = 2 * BPS;
    uint256 private constant FLOW_REBATE_BPS = 3;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 varSlow = slots[12];
        uint256 lambdaSlow = slots[13];
        ...
        if (varSlow == 0) varSlow = varEWMA;
        if (lambdaSlow == 0) lambdaSlow = lambdaEWMA;
        ...
        if (likelyArb) {
            ...
            varSlow = _ewma(varSlow, varEWMA, ALPHA_VAR_SLOW / 2);
        }
        ...
        uint256 sigmaFast = sqrt(varEWMA * WAD);
        ...
        sigma = ((sigmaFast * 65) + (sigmaSlow * 35)) / 100;
        sigmaBps = sigma / BPS;

        uint256 lambdaBlend = (lambdaEWMA * 2 + lambdaSlow) / 3;
        uint256 safeBps = SAFE_BASE_BPS + ((sigma * SAFE_VOL_MULT_BPS) / WAD) + shockBps;
        if (lambdaBlend > 0) {
            uint256 lambdaNum = LAMBDA_REF + ARMOR_LAMBDA;
            uint256 lambdaDen = lambdaBlend + ARMOR_LAMBDA;
        ...
        if (sigmaFast > sigmaSlow + SIGMA_STRESS_GAP) {
            safeBps += 1;
            vulnFloor += 2;
        }
        if (shockBps == 0 && lambdaEWMA > lambdaSlow) {
            safeBps = safeBps > FLOW_REBATE_BPS ? (safeBps - FLOW_REBATE_BPS) : MIN_FEE_BPS;
        }

        if (lambdaBlend < ARMOR_LAMBDA && sigma > ARMOR_SIGMA) {
