...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant CONT_RETAIL_COMPETE_BPS = 4;
    ...
    // [10] ratioEWMA
    // [11] lastArbTs
    // [12] stepToxic (1 if first trade likely arb)
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (firstInStep) slots[12] = likelyArb ? 1 : 0;
        ...
        // If the first trade of the step was retail-like, continuation flow is usually safer.
        if (!firstInStep && slots[12] == 0 && shockBps == 0) {
            uint256 contCut = CONT_RETAIL_COMPETE_BPS;
            if (lambdaEWMA > LAMBDA_REF) contCut += 1;
            bidBps = bidBps > contCut ? (bidBps - contCut) : MIN_FEE_BPS;
            askBps = askBps > contCut ? (askBps - contCut) : MIN_FEE_BPS;
        }
