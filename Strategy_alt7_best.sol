...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant ARB_BASE_RATIO_WAD = 19 * BPS;
    ...
    uint256 private constant ALPHA_L = 12e16; // slow flow EWMA
    uint256 private constant ALPHA_L_FAST = 21e16;
    uint256 private constant ALPHA_RATIO = 14e16;
    ...
    uint256 private constant SAFE_VOL_MULT_BPS = 1979;
    uint256 private constant SAFE_LAMBDA_SWING_BPS = 9;
    uint256 private constant SAFE_SIGMA_SQ_DIV = 85;
    uint256 private constant SAFE_RATIO_DIV = 42;
    uint256 private constant FLOW_RECOVER_BPS = 7;
    uint256 private constant FLOW_MONETIZE_BPS = 3;

    uint256 private constant VULN_MIN_BPS = 51;
    uint256 private constant VULN_SIGMA_DIV = 17;
    uint256 private constant VULN_BUFFER_BPS = 1;
    uint256 private constant VULN_RATIO_DIV = 43;
    ...
    uint256 private constant INV_SENS_BPS = 116;
    ...
    uint256 private constant RATIO_REBATE_GUARD_BPS = 52;

    uint256 private constant SHOCK_RATIO_WAD = 92 * BPS;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (firstInStep && lastTs > 0 && trade.timestamp > lastTs) {
            ...
            uint256 inst = WAD / dt;
            lambdaSlow = _ewma(lambdaSlow, inst, ALPHA_L);
        ...
        uint256 ratioBps = ratioEWMA / BPS;
        uint256 safeBps = SAFE_BASE_BPS + ((sigma * SAFE_VOL_MULT_BPS) / WAD) + shockBps;
        safeBps += (sigmaBps * sigmaBps) / SAFE_SIGMA_SQ_DIV;
        safeBps += ratioBps / SAFE_RATIO_DIV;
        if (lambdaSlow > 0) {
        ...
        vulnFloor += ratioBps / VULN_RATIO_DIV;
        ...
        if (firstInStep && likelyArb && shockBps == 0 && ratioBps <= RATIO_REBATE_GUARD_BPS) {
        ...
        } else if (!firstInStep && stepTrades >= 2 && shockBps == 0 && ratioBps <= RATIO_REBATE_GUARD_BPS) {
