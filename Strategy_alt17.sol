...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant ARB_PROB_CUTOFF = 55e16;
    uint256 private constant ARB_PROB_STRONG = 68e16;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (firstInStep) {
            arbProb = 45e16;
            if (tradeRatio <= arbCap) arbProb += 24e16;
            else arbProb = arbProb > 20e16 ? (arbProb - 20e16) : 0;

            if (aligned) arbProb += 26e16;
            else arbProb = arbProb > 24e16 ? (arbProb - 24e16) : 0;

            if (tradeRatio <= arbCap / 2) arbProb += 12e16;
            if (arbProb > WAD) arbProb = WAD;
        }
        bool likelyArb = arbProb >= ARB_PROB_CUTOFF;
        stepTrades = firstInStep ? 1 : (stepTrades + 1);

        if (firstInStep && lastTs > 0 && trade.timestamp > lastTs) {
            uint256 dt = trade.timestamp - lastTs;
            uint256 inst = WAD / dt;
            uint256 alphaL = wmul(ALPHA_L, WAD - arbProb);
            lambdaEWMA = _ewma(lambdaEWMA, inst, alphaL);
        }
        ...
        if (firstInStep) {
            ...
            uint256 alphaP = wmul(ALPHA_P, arbProb);
            if (alphaP > 0) pHat = _ewma(pHat, pEst, alphaP);

            uint256 alphaVar = wmul(ALPHA_VAR, arbProb);
            if (alphaVar > 0) {
                ...
                lastFair = _ewma(lastFair, pEst, alphaVar);
            }
            if (arbProb >= ARB_PROB_STRONG) lastArbTs = trade.timestamp;
        }
        else {
