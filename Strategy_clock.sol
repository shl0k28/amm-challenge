...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant INTRASTEP_REBATE_BPS = 9;
    uint256 private constant INTRASTEP_LAMBDA_BONUS_BPS = 10;
    uint256 private constant INTRASTEP_MAX_REBATE_BPS = 22;
    uint256 private constant CARRY_REARM_BPS = 8;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (firstInStep && shockBps == 0) {
            uint256 intra = INTRASTEP_REBATE_BPS;
            if (lambdaEWMA > LAMBDA_REF) {
                uint256 ex = wdiv(lambdaEWMA, LAMBDA_REF) - WAD;
                uint256 bonus = (ex * INTRASTEP_LAMBDA_BONUS_BPS) / WAD;
                if (bonus > INTRASTEP_LAMBDA_BONUS_BPS) bonus = INTRASTEP_LAMBDA_BONUS_BPS;
                intra += bonus;
            }
            if (likelyArb) intra += 2;
            if (intra > INTRASTEP_MAX_REBATE_BPS) intra = INTRASTEP_MAX_REBATE_BPS;
            bidBps = bidBps > intra ? (bidBps - intra) : MIN_FEE_BPS;
            askBps = askBps > intra ? (askBps - intra) : MIN_FEE_BPS;
        } else if (!firstInStep) {
