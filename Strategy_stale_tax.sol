...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant STALE_WARN_STEPS = 4;
    uint256 private constant STALE_SAFE_PER_STEP = 1;
    uint256 private constant STALE_VULN_PER_STEP = 2;
    uint256 private constant STALE_SAFE_MAX = 8;
    uint256 private constant STALE_VULN_MAX = 16;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (lastArbTs > 0 && trade.timestamp > lastArbTs + STALE_WARN_STEPS) {
            uint256 staleSteps = trade.timestamp - lastArbTs - STALE_WARN_STEPS;
            uint256 staleSafe = staleSteps * STALE_SAFE_PER_STEP;
            uint256 staleVuln = staleSteps * STALE_VULN_PER_STEP;
            if (staleSafe > STALE_SAFE_MAX) staleSafe = STALE_SAFE_MAX;
            if (staleVuln > STALE_VULN_MAX) staleVuln = STALE_VULN_MAX;
            safeBps += staleSafe;
            vulnFloor += staleVuln;
        }
