...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant CALM_SAFE_CUT_BPS = 5;
    uint256 private constant CALM_VULN_CUT_BPS = 4;
    uint256 private constant STORM_SIGMA = 11 * BPS;
    uint256 private constant STORM_LAMBDA = 28e16;
    uint256 private constant STORM_SAFE_ADD_BPS = 5;
    uint256 private constant STORM_VULN_ADD_BPS = 8;
    ...
    uint256 private constant SHIELD_REBATE_BPS = 8;
    ...
    uint256 private constant STALE_STEP = 30;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (calm) {
            safeBps = safeBps > CALM_SAFE_CUT_BPS ? (safeBps - CALM_SAFE_CUT_BPS) : MIN_FEE_BPS;
        } else if (storm) {
            safeBps += STORM_SAFE_ADD_BPS;
        }
        ...
        if (calm) {
            vulnFloor = vulnFloor > CALM_VULN_CUT_BPS ? (vulnFloor - CALM_VULN_CUT_BPS) : MIN_FEE_BPS;
        } else if (storm) {
            vulnFloor += STORM_VULN_ADD_BPS;
        }
        vulnFloor += staleBps;
        ...
        if (pHat > 0) {
            uint256 shieldRebate = SHIELD_REBATE_BPS;
            if (storm || staleBps >= 4) shieldRebate /= 2;
            else if (calm) shieldRebate += 2;
            if (spot < pHat) {
                ...
                askBps = reqAskBps;
                bidBps = bidBps > shieldRebate ? (bidBps - shieldRebate) : MIN_FEE_BPS;
            } else if (spot > pHat) {
                ...
                bidBps = reqBidBps;
                askBps = askBps > shieldRebate ? (askBps - shieldRebate) : MIN_FEE_BPS;
            }
        }
        ...
        if (firstInStep && likelyArb && shockBps == 0 && staleBps < 5) {
        ...
        } else if (!firstInStep && stepTrades >= 2 && shockBps == 0 && !storm && staleBps < 5) {
