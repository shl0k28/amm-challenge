...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant VOLACC_STEP_DIV = 6;
    uint256 private constant VOLACC_DECAY = 1;
    uint256 private constant VOLACC_MAX = 54;
    uint256 private constant VOLACC_QUAD_DIV = 18;
    uint256 private constant VOLACC_SAFE_DIV = 2;
    uint256 private constant VOLACC_EXTRA_MAX = 28;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (likelyArb) {
            ...
            uint256 diffBps = diff / BPS;
            uint256 bump = 1 + (diffBps / VOLACC_STEP_DIV);
            volAcc += bump;
            if (volAcc > VOLACC_MAX) volAcc = VOLACC_MAX;
            lastFair = pEst;
            lastArbTs = trade.timestamp;
        } else {
            pHat = _ewma(pHat, spot, 5e16);
            if (volAcc > VOLACC_DECAY) volAcc -= VOLACC_DECAY;
            else volAcc = 0;
        }
        ...
        uint256 volExtraBps = (volAcc * volAcc) / VOLACC_QUAD_DIV;
        if (volExtraBps > VOLACC_EXTRA_MAX) volExtraBps = VOLACC_EXTRA_MAX;

        uint256 safeBps = SAFE_BASE_BPS + ((sigma * SAFE_VOL_MULT_BPS) / WAD) + shockBps + (volExtraBps / VOLACC_SAFE_DIV);
        if (lambdaEWMA > 0) {
        ...
        vulnFloor += staleBps * STALE_VULN_PER_BPS + volExtraBps;
