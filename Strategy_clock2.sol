...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant CONT_ARB_REBATE_BPS = 8;
    uint256 private constant CONT_TAIL_REBATE_BPS = 3;
    uint256 private constant FIRST_RETAIL_REBATE_BPS = 11;
    uint256 private constant CARRY_BASE_BPS = 2;
    uint256 private constant CARRY_SIGMA_DIV = 9;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (firstInStep && shockBps == 0) {
            uint256 r = likelyArb ? CONT_ARB_REBATE_BPS : FIRST_RETAIL_REBATE_BPS;
        ...
        uint256 carryBps = CARRY_BASE_BPS + (sigmaBps / CARRY_SIGMA_DIV);
        if (firstInStep && shockBps == 0) carryBps = 0;
        bidFee += bpsToWad(carryBps);
        askFee += bpsToWad(carryBps);
