...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant FIRST_RETAIL_REBATE_BPS = 2;
    uint256 private constant FIRST_RETAIL_LAMBDA_BONUS_BPS = 2;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (firstInStep && shockBps == 0 && staleBps <= STALE_REBATE_GUARD_BPS) {
            if (likelyArb) {
                ...
                uint256 extra = FIRST_RETAIL_REBATE_BPS;
                if (lambdaEWMA > LAMBDA_REF) {
                    uint256 ex = wdiv(lambdaEWMA, LAMBDA_REF) - WAD;
                    uint256 bonus = (ex * FIRST_RETAIL_LAMBDA_BONUS_BPS) / WAD;
                    if (bonus > FIRST_RETAIL_LAMBDA_BONUS_BPS) bonus = FIRST_RETAIL_LAMBDA_BONUS_BPS;
                    extra += bonus;
                }
                bidBps = bidBps > extra ? (bidBps - extra) : MIN_FEE_BPS;
                askBps = askBps > extra ? (askBps - extra) : MIN_FEE_BPS;
            }
