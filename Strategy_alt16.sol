...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant ARB_MAX_CAP_WAD = 91 * BPS;
    uint256 private constant ALPHA_P = 36e16;
    uint256 private constant ALPHA_VAR = 21e16;
    ...
    uint256 private constant SAFE_VOL_MULT_BPS = 2060;
    ...
    uint256 private constant CALM_SAFE_SHIFT_BPS = 0;
    ...
    uint256 private constant SHOCK_RATIO_WAD = 95 * BPS;
    uint256 private constant SHOCK_BUMP_BPS = 6;
    uint256 private constant SHOCK_DECAY_BPS = 2;
    uint256 private constant SHOCK_MAX_BPS = 17;
    uint256 private constant HAZ_DECAY_BPS = 1;
    uint256 private constant HAZ_ARB_DROP_BPS = 3;
    uint256 private constant HAZ_FIRST_RISE_BPS = 4;
    uint256 private constant HAZ_SHOCK_RISE_BPS = 2;
    uint256 private constant HAZ_STALE_STEP = 28;
    uint256 private constant HAZ_MAX_BPS = 20;
    uint256 private constant MODE_ON_BPS = 11;
    uint256 private constant MODE_OFF_BPS = 4;
    uint256 private constant MODE_SAFE_ADD_BPS = 7;
    uint256 private constant MODE_VULN_ADD_BPS = 11;
    uint256 private constant MODE_REBATE_BOOST_BPS = 1;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (hazardBps > HAZ_DECAY_BPS) hazardBps -= HAZ_DECAY_BPS;
        else hazardBps = 0;
        if (likelyArb) {
            hazardBps = hazardBps > HAZ_ARB_DROP_BPS ? (hazardBps - HAZ_ARB_DROP_BPS) : 0;
        } else if (firstInStep) {
            hazardBps += HAZ_FIRST_RISE_BPS;
        }
        if (tradeRatio >= SHOCK_RATIO_WAD) hazardBps += HAZ_SHOCK_RISE_BPS;
        if (lastArbTs > 0 && trade.timestamp > lastArbTs) hazardBps += (trade.timestamp - lastArbTs) / HAZ_STALE_STEP;
        if (hazardBps > HAZ_MAX_BPS) hazardBps = HAZ_MAX_BPS;

        if (mode == 0 && hazardBps >= MODE_ON_BPS) mode = 1;
        else if (mode == 1 && hazardBps <= MODE_OFF_BPS) mode = 0;
        ...
        if (mode == 1) safeBps += MODE_SAFE_ADD_BPS;
        else safeBps = safeBps > MODE_REBATE_BOOST_BPS ? (safeBps - MODE_REBATE_BOOST_BPS) : MIN_FEE_BPS;
        ...
        if (mode == 1) vulnFloor += MODE_VULN_ADD_BPS;
        ...
        if (mode == 0) {
            uint256 arbRebate = CONT_ARB_REBATE_BPS + MODE_REBATE_BOOST_BPS;
            uint256 tailRebate = CONT_TAIL_REBATE_BPS + MODE_REBATE_BOOST_BPS;
