...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        bool strongArb = firstInStep && (tradeRatio <= arbCap / 2) && aligned;
        bool weakArb = firstInStep && (tradeRatio <= arbCap) && (aligned || tradeRatio <= ratioEWMA / 2);
        bool likelyArb = strongArb || weakArb;
        ...
        if (likelyArb) {
            ...
            uint256 alphaP = strongArb ? ALPHA_P : (ALPHA_P * 2) / 3;
            pHat = _ewma(pHat, pEst, alphaP);

            uint256 ratio = lastFair > 0 ? wdiv(pEst, lastFair) : WAD;
            uint256 diff = ratio > WAD ? (ratio - WAD) : (WAD - ratio);
            uint256 alphaVar = strongArb ? ALPHA_VAR : (ALPHA_VAR * 2) / 3;
            varEWMA = _ewma(varEWMA, wmul(diff, diff), alphaVar);
            lastFair = _ewma(lastFair, pEst, alphaP);
            if (strongArb) lastArbTs = trade.timestamp;
        } else {
