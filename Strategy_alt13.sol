...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant HAZ_ARB_RELIEF_BPS = 4;
    uint256 private constant HAZ_FIRST_PENALTY_BPS = 5;
    uint256 private constant HAZ_SHOCK_BPS = 4;
    uint256 private constant HAZ_MIS_THRESH_BPS = 12;
    uint256 private constant HAZ_MIS_DIV = 4;
    uint256 private constant HAZ_STALE_DIV = 48;
    uint256 private constant HAZ_MAX_BPS = 26;
    uint256 private constant HAZ_SAFE_DIV = 2;
    uint256 private constant HAZ_REBATE_CUTOFF = 9;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (likelyArb) {
            hazardBps = hazardBps > HAZ_ARB_RELIEF_BPS ? (hazardBps - HAZ_ARB_RELIEF_BPS) : 0;
        } else if (firstInStep) {
            hazardBps += HAZ_FIRST_PENALTY_BPS;
        }
        if (tradeRatio >= SHOCK_RATIO_WAD) hazardBps += HAZ_SHOCK_BPS;
        if (misBps > HAZ_MIS_THRESH_BPS) hazardBps += (misBps - HAZ_MIS_THRESH_BPS) / HAZ_MIS_DIV;
        if (lastArbTs > 0 && trade.timestamp > lastArbTs) hazardBps += (trade.timestamp - lastArbTs) / HAZ_STALE_DIV;
        ...
        safeBps += hazardBps / HAZ_SAFE_DIV;
        ...
        if (firstInStep && likelyArb && shockBps == 0 && staleBps <= STALE_REBATE_GUARD_BPS && hazardBps <= HAZ_REBATE_CUTOFF) {
            ...
            !firstInStep && stepTrades >= 2 && shockBps == 0 && staleBps <= STALE_REBATE_GUARD_BPS
                && hazardBps <= HAZ_REBATE_CUTOFF
        ) {
