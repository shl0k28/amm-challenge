...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 streakSide = slots[11];
        uint256 streakLen = slots[12];
        ...
        if (streakLen >= 3) {
            uint256 bump = streakLen - 2;
            if (bump > 4) bump = 4;
            if (streakSide == 1) bidBps += bump;
            else if (streakSide == 2) askBps += bump;
        }
