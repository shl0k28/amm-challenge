...
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant CARRY_BASE = 5 * BPS;
    uint256 constant CARRY_TOX_COEF = 1200 * BPS;
    uint256 constant POST_FIRST_DISCOUNT = 3 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 carryPrem = CARRY_BASE + wmul(CARRY_TOX_COEF, toxSignal);
        if (firstInStep) {
            bidFee = bidFee > POST_FIRST_DISCOUNT ? (bidFee - POST_FIRST_DISCOUNT) : 0;
            askFee = askFee > POST_FIRST_DISCOUNT ? (askFee - POST_FIRST_DISCOUNT) : 0;
        } else {
            bidFee += carryPrem;
            askFee += carryPrem;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "BandShield_ghost_carryclock";
    }
}
