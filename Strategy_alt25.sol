...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant ARB_BASE_RATIO_WAD = 15 * BPS;
    uint256 private constant ARB_RATIO_DIV = 1;
    uint256 private constant ARB_SIGMA_DIV = 6;
    ...
    uint256 private constant RECON_END_STEP = 120;
    uint256 private constant RECON_SAFE_ADD = 4;
    uint256 private constant RECON_VULN_ADD = 4;
    uint256 private constant ENDGAME_START_STEP = 9200;
    uint256 private constant ENDGAME_SAFE_ADD = 2;
    uint256 private constant ENDGAME_VULN_ADD = 2;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (!likelyArb && firstInStep && lastTs > 0 && trade.timestamp > lastTs) {
        ...
        if (trade.timestamp < RECON_END_STEP) {
            safeBps += RECON_SAFE_ADD;
            vulnFloor += RECON_VULN_ADD;
        } else if (trade.timestamp > ENDGAME_START_STEP) {
            safeBps += ENDGAME_SAFE_ADD;
            vulnFloor += ENDGAME_VULN_ADD;
        }
